      ...
      <div className="container-custom relative z-10">
        <Reveal>
          <div className="mb-6 md:mb-16 relative">
            <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold tracking-tighter text-text-main">
          ...
          <div className="relative border-l border-border/20 ml-2 lg:ml-40 pr-4 lg:pr-0">
            ...
            {experience.map((job, idx) => (
              <Reveal key={job.id} delay={idx * 0.1}>
                <div className="relative pl-8 lg:pl-16 pb-8 md:pb-20 last:pb-2 last:md:pb-10 group/experience">
                  {/* Date Range - Desktop */}
                  ...
                  <div className="space-y-3 md:space-y-8">
                    <div className="space-y-2 md:space-y-3 relative">
                      <div className="flex items-center gap-4 flex-wrap">
                        <h3 className="text-base md:text-xl font-bold text-text-main tracking-tight group-hover/experience:text-cyan-700 dark:group-hover/experience:text-white transition-colors">
                          <CodeText tag="h3">{job.title[loc]}</CodeText>
                        </h3>
                      </div>
                      <div className="text-xs text-cyan-600 dark:text-cyan-400 font-bold tracking-[0.25em] uppercase flex items-center gap-3">
                    ...
                    <ul className="space-y-3 md:space-y-6 max-w-2xl relative">
                      {job.points[loc].map((point, pIdx) => (
                        <li key={`job-point-${pIdx}`} className="flex gap-3 md:gap-6 group/point relative">
                          ...
                          <p className="text-sm md:text-base leading-relaxed text-text-muted transition-all duration-300 border-l border-border/30 pl-4 hover:border-cyan-500/20">
