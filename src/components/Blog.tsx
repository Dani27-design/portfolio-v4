...
import { LogStreamGimmick } from "./LogStreamGimmick";
...
export const Blog = () => {
  ...
  // Get translated blog entries
  const getBlogEntry = (slug: string) => {
    const entries = t('blogEntries') as Record<string, { title: string; excerpt: string }>;
    return entries[slug] || { title: '', excerpt: '' };
  };
      ...
      <div className="container-custom relative z-10">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-24 gap-8 items-center md:items-end">
          <Reveal>
            <div className="relative flex flex-col items-center md:items-start text-center md:text-left">
              <h2 className="text-3xl font-bold tracking-tighter text-text-main md:text-5xl lg:text-6xl">{t('blog.title')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {BLOGS.slice(0, 3).map((blog, idx) => {
            ...
            return (
              <Reveal key={blog.slug} delay={idx * 0.1} width="100%">
                <Link to={localizedPath(`/blog/${blog.slug}`)} className="block h-full">
                  <motion.div
                    whileHover={{ y: -8 }}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-10 relative z-10">
                       <div className="flex flex-col gap-2">
                          ...
                          <span className="text-[7px] font-mono text-white/20 uppercase tracking-tighter">COMMIT_HASH: 0x{Math.random().toString(16).substring(2, 10).toUpperCase()}</span>
                       </div>
