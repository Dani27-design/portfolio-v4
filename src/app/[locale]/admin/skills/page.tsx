  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6 lg:mb-8">
        <h1 className="text-lg md:text-xl font-bold text-white">Skills</h1>
        <button
          ...
          <div key={g.id} className="bg-slate-800 border border-slate-700 rounded-lg p-4">
            <div className="flex justify-between items-start">
              <div className="flex-1 min-w-0 mr-3">
                ...
                <div className="text-slate-400 text-xs mt-1 truncate">{g.skills.map(s => s.name).join(', ')}</div>
              </div>
  ...
  return (
    <div className="mb-6 lg:mb-8 bg-slate-800 border border-slate-700 rounded-lg p-4 lg:p-6">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs text-slate-400 mb-1">Context (EN)</label>
          <textarea value={form.contextEn} onChange={e => setForm({...form, contextEn: e.target.value})} placeholder="Context (EN)" rows={2} className={inputClass} />
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1">Context (ID)</label>
          <textarea value={form.contextId} onChange={e => setForm({...form, contextId: e.target.value})} placeholder="Context (ID)" rows={2} className={inputClass} />
        </div>
        <div className="md:col-span-2">
          <label className="block text-xs text-slate-400 mb-1">Skills (one per line, format: Name:TAG)</label>
          <textarea value={form.skills} onChange={e => setForm({...form, skills: e.target.value})} placeholder={"TypeScript:STRICT\nNode.js:RUNTIME"} rows={5} className={inputClass + " font-mono"} />
        </div>
        <div>
